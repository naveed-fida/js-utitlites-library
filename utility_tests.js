...
test("pick can return an object with more than one properties", function() {
  var old_obj = {foo: 1, bar: 2, baz: 3, bac: 4},
      new_obj = _(old_obj).pick("foo", "baz");
      console.log(new_obj);
  return new_obj.foo === 1 && new_obj.baz === 3 && new_obj.bac === undefined;
});
...
test("omit returns a new object with any passed in properties omitted", function() {
  var old_obj = { foo: "bar", baz: "okay" },
      new_obj = _(old_obj).omit("foo");

  return new_obj.foo === undefined && new_obj.baz === "okay";
});
...
test("object.isElement returns true if DOM element, otherwise false", function() {
  return _(document.body).isElement() && !_({}).isElement();
});
...
test("object.isArray returns true if array, otherwise false", function() {
...
test("object.isObject returns true if object or function, otherwise false", function() {
  return _({}).isObject() && _([]).isObject() && _(isNaN).isObject() && !_(1).isObject();
});
...
test("object.isFunction returns true if function, otherwise false", function() {
...
test("object.isBoolean returns true if boolean (primitive or object), otherwise false", function() {
...
test("object.isString returns true if string, otherwise false", function() {
...
test("object.isNumber second way returns true if number, (primitive or object), otherwise false", function() {
